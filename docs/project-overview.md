...
```mermaid
graph TB
    subgraph BrowserTab["Browser Tab (Web Page)"]
        WebPage["Web Page"]
    end

    subgraph ServiceWorker["Service Worker (background.ts)"]
        Alarms["chrome.alarms<br/>(≥ 60s)"]
        SetTimeout["setTimeout<br/>(< 60s)"]
        Alarms --> TimerFire
        SetTimeout --> TimerFire
        TimerFire["onAlarm / onTimeout"]
        TimerFire -->|"chrome.tabs.reload(tabId)"| WebPage
        Storage["chrome.storage.local<br/>{ tabId: { interval, unit, ... } }"]
        Cleanup["chrome.tabs.onRemoved<br/>→ cleanup alarm + storage"]
    end

    subgraph PopupUI["Popup UI (popup.html/ts)"]
    ...
    PopupUI -->|"messages<br/>(start/stop)"| ServiceWorker
    ServiceWorker -->|"messages<br/>(status)"| PopupUI

    style BrowserTab fill:#e3f2fd,stroke:#1565c0,color:#000
    style ServiceWorker fill:#fff3e0,stroke:#e65100,color:#000
    style PopupUI fill:#e8f5e9,stroke:#2e7d32,color:#000
    style TimerFire fill:#ffcc80,stroke:#e65100,color:#000
    style WebPage fill:#bbdefb,stroke:#1565c0,color:#000
    style Storage fill:#ffe0b2,stroke:#e65100,color:#000
    style Cleanup fill:#ffe0b2,stroke:#e65100,color:#000
    style Alarms fill:#ffcc80,stroke:#e65100,color:#000
    style SetTimeout fill:#ffcc80,stroke:#e65100,color:#000
    style Input fill:#c8e6c9,stroke:#2e7d32,color:#000
    style Buttons fill:#c8e6c9,stroke:#2e7d32,color:#000
    style Status fill:#c8e6c9,stroke:#2e7d32,color:#000
```
